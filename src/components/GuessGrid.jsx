...
export default function GuessGrid({ wordLength, pastGuesses, maxGuesses = 6, currentInput = '', shake = null }) {
  ...
  for (const past of pastGuesses) {
    rows.push({
      kind: 'past',
      cells: past.feedback.map((fb, i) => ({ letter: past.guess?.[i] ?? '', feedback: fb })),
    })
  }
  ...
  while (rows.length < maxGuesses) {
    rows.push({ kind: 'blank', cells: buildBlankRow(wordLength) })
  }
  ...
  return (
    <div className="guess-grid">
      {rows.map((row, r) => (
        <div
          className={`guess-row${row.kind === 'typed' ? ' guess-row--typed' : ''}`}
          key={r}
